...
**Current Acceptable Pedantic Allows (24 total):**

These `#[allow(clippy::...)]` annotations are justified and documented in code (as of 2025-12-21):
...
- `items_after_statements` (2×) → Constants scoped in spawn blocks for clarity (some combined with `too_many_lines` on same line)
...
- `match_same_arms` (1×) → Rule editor input - conceptually different field types despite similar-looking actions (combined with `too_many_lines`)
- `needless_raw_string_hashes` (1×) → Generated by build.rs (built_info crate)
- `doc_markdown` (1×) → Generated by build.rs (built_info crate)

**Note:** Several `#[allow]` lines combine multiple suppressions (e.g., `#[allow(clippy::too_many_lines, clippy::items_after_statements)]`), bringing the total to 24 attribute lines. See `rust_plan.md` for detailed breakdown and migration plan to centralize these into Cargo.toml `[lints]` table.
