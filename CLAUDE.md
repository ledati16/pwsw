...
# Pedantic mode (current state: ~1 trivial warning)
cargo clippy --all-targets -- -W clippy::pedantic
```

**Current Acceptable Pedantic Allows (as of latest refactor):**

These `#[allow(clippy::...)]` annotations are justified and documented in code:
- `struct_excessive_bools` (3×) → Config `Settings` and TUI `App` legitimately need multiple independent boolean flags
- `too_many_lines` (10×) → Daemon event loop, TUI screens, commands - cohesive functions hard to split meaningfully
- `items_after_statements` (2×) → Constants defined in spawn blocks for scoping
- `needless_pass_by_value` (1×) → Wayland `Connection` must be moved, not borrowed (protocol requirement)
- `too_many_arguments` (1×) → TUI render function with legitimate parameter count
- Minor allows for specific justified cases (see code comments)

**Policy:** Prefer fixing warnings over adding new `#[allow(...)]` attributes. Each `#[allow]` must have a comment explaining why it's necessary.
