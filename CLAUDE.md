...
**Acceptable Pedantic Warnings (8 total):**
...
- 2× `too_many_lines` → Daemon event loop (150 lines) and list_sinks command (112 lines) are cohesive and hard to split meaningfully
- 2× `items_after_statements` → Constants defined in spawn block for scoping
- 2× `needless_pass_by_value` → Wayland `Connection` must be moved, not borrowed (protocol requirement)
