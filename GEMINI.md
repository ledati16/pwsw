...
- **Read First:** Before performing any task, read `refactor_review_final.md` to understand the current phase and objectives. Note that the rest of `GEMINI.md` is just as important and needs to be read as well before implementing any changes.
- **Update Constanty:** After completing a step, mark it as checked `[x]` in `refactor_review_final.md`. If you change the approach, update the plan text.
- **Goal:** Modernize the codebase using ecosystem crates (`tui-input`, `notify`, `color-eyre`) to reduce boilerplate and fragility.
...
**IMPORTANT**: This codebase was entirely generated by LLMs without peer review from experienced Rust developers. However, it has undergone comprehensive code review and pedantic clippy cleanup (154 warnings → 8 acceptable). Exercise caution when making changes and validate thoroughly.
...
- 2× `struct_excessive_bools` → Settings struct legitimately needs 5 independent boolean flags (default_on_startup, set_smart_toggle, notify_manual, notify_rules, match_by_index)
- 2× `too_many_lines` → Daemon event loop (154 lines) and list_sinks command (117 lines) are cohesive and hard to split meaningfully
- 2× `items_after_statements` → Constants defined in spawn block for scoping
- 1× `needless_pass_by_value` → Wayland `Connection` must be moved, not borrowed (protocol requirement)
...
- ✅ `PipeWire`, `app_id`, `IPC`, `XDG_RUNTIME_DIR`, `pw-dump`, `crossterm`
- ❌ PipeWire, app_id, IPC, XDG_RUNTIME_DIR, pw-dump, crossterm
